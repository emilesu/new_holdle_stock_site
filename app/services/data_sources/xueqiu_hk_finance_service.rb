  ...
  class XueqiuHkFinanceService
    ...
    class << self
      def call
        ...
        pending_stocks = Stock.where(market: "HK")
      ...
      def call_single(symbol)
        ...
        results << { name: "利润表", result: execute_service("利润表") { XueqiuIncomeStatementService.call(symbol, market: "HK") } }
        results << { name: "资产负债表", result: execute_service("资产负债表") { XueqiuBalanceSheetService.call(symbol, market: "HK") } }
        results << { name: "现金流量表", result: execute_service("现金流量表") { XueqiuCashFlowService.call(symbol, market: "HK") } }
        results << { name: "财务指标", result: execute_service("财务指标") { XueqiuIndicatorService.call(symbol, market: "HK") } }
