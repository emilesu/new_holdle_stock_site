...
class StockScreenerService
  ...
  def parse!
    ...
    unless @year_from.between?(MIN_YEAR, latest_year) && @year_to.between?(MIN_YEAR, latest_year) && @year_from <= @year_to
      @result.errors << "年度区间无效（须在 #{MIN_YEAR} ~ #{latest_year} 之间，且起始年不晚于结束年）"
      return
    end
    if @year_to - @year_from + 1 > MAX_SPAN
      @result.errors << "年度区间跨度不能超过 #{MAX_SPAN} 年"
      return
    end
    ...
    if @growth_mode == 'yoy'
      if @growth_value.nil? || !@growth_value.between?(-100.0, 1000.0)
        @result.errors << '请填写有效的净利润同比增长阈值（-100 ~ 1000）'
        return
      end
    elsif @growth_mode == 'always_up' && @year_span < 2
      @result.errors << '「区间内逐年上升」需要至少 2 个年度'
      return
    end

    if @margin_conditions.empty? && @growth_mode.nil?
      @result.errors << '请至少设置一个筛选条件'
      return
    end
  ...
  def margin_ids_sql
    ...
    when 'all'
      ...
      @margin_conditions.each do |c|
        having << "COUNT(#{c[:column]}) = #{@year_span.to_i}"
        if c[:op] == 'gte'
          having << "MIN(#{c[:column]}) >= ?"
        else
          having << "MAX(#{c[:column]}) <= ?"
  ...
  def growth_ids_sql
    ...
    when 'always_up'
      sql = "SELECT stock_id FROM (" \
            "SELECT stock_id, net_income_to_shareholders AS v, " \
            ...
            ") t GROUP BY stock_id " \
            "HAVING COUNT(*) = ? AND SUM(CASE WHEN v IS NULL THEN 1 ELSE 0 END) = 0 " \
