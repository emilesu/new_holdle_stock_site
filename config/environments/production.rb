...
Rails.application.configure do
  ...
  # Nginx 处理 SSL 终结，内部 Puma 始终通过 HTTP 接收请求
  # 启用后 Rails 会认为所有请求都是 HTTPS（secure cookie、HSTS 等行为正确）
  ...
  config.active_job.queue_name_prefix = "stock_website_production"
  ...
  # T7 Phase3：Active Record 加密（api_keys.key_plaintext）密钥从 .env.production 读取（缺失即报错）
